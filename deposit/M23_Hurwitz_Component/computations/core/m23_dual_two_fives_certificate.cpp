...
int main(){
 ...
 // Point-push subgroup generated by the four mixed collision operators.
 vector<vector<int>> pp=chroms;
 for(auto const&p:chroms) pp.push_back(invperm(p));
 vector<int> orb_id(states.size(),-1); vector<vector<int>> pp_orbits;
 for(int s=0;s<(int)states.size();s++) if(orb_id[s]<0){
   int oid=pp_orbits.size(); vector<int> O; queue<int> qb; qb.push(s);orb_id[s]=oid;
   while(!qb.empty()){int v=qb.front();qb.pop();O.push_back(v);for(auto const&p:pp){int w=p[v];if(orb_id[w]<0){orb_id[w]=oid;qb.push(w);}}}
   sort(O.begin(),O.end()); pp_orbits.push_back(move(O));
 }
 map<int,int> pphist; for(auto const&O:pp_orbits)pphist[(int)O.size()]++;
 cout<<"POINT_PUSH_ORBITS "<<pp_orbits.size()<<" SIZE_HIST";for(auto[k,v]:pphist)cout<<" "<<k<<":"<<v;cout<<"\n";
 // Test all generator orders for the peripheral product relation.
 array<int,4> ords={0,1,2,3}; int relcount=0;
 do{vector<int> prod(states.size());std::iota(prod.begin(),prod.end(),0);for(int a:ords)prod=comp(chroms[a],prod);bool ok=true;for(int i=0;i<(int)prod.size();i++)if(prod[i]!=i){ok=false;break;}if(ok){relcount++;cout<<"POINT_PUSH_REL_ORDER "<<ords[0]<<ords[1]<<ords[2]<<ords[3]<<"\n";}}while(next_permutation(ords.begin(),ords.end()));
 cout<<"POINT_PUSH_REL_COUNT "<<relcount<<"\n";
 // Riemann-Hurwitz ledgers for every point-push orbit.
 map<long long,int> genus_hist; int invalid=0; int orbno=0;
 for(auto const&O:pp_orbits){
   unordered_map<int,int> loc;loc.reserve(O.size()*2);for(int i=0;i<(int)O.size();i++)loc[O[i]]=i;
   long long sumc=0; vector<int> cs;
   for(auto const&p:chroms){vector<char>seen(O.size());int cnum=0;for(int ii=0;ii<(int)O.size();ii++)if(!seen[ii]){cnum++;int v=ii;while(!seen[v]){seen[v]=1;v=loc.at(p[O[v]]);}}cs.push_back(cnum);sumc+=cnum;}
   long long d=O.size(); long long num=2+2*d-sumc; // 2g = 2+2d-sum c_i
   if(num%2){invalid++;}
   long long g=num/2;genus_hist[g]++;
   if(pp_orbits.size()<=20 || d>=1000){cout<<"POINT_PUSH_ORBIT "<<orbno<<" D "<<d<<" CYCLES";for(int c:cs)cout<<" "<<c;cout<<" GENUS "<<g<<"\n";}
   orbno++;
 }
 cout<<"POINT_PUSH_GENUS_HIST";for(auto[k,v]:genus_hist)cout<<" "<<k<<":"<<v;cout<<" INVALID_PARITY "<<invalid<<"\n";

 // Exact stabilizer verification for the selected ten-state cell D = Fix(chroms[0]) intersect width_5(monos[0]).
 vector<int> D10; for(int i=0;i<(int)states.size();i++) if(mw[0][i]==5 && cf[0][i]) D10.push_back(i);
 sort(D10.begin(),D10.end());
 cout<<"D10_SIZE "<<D10.size()<<" STATES"; for(int v:D10) cout<<" "<<v; cout<<"\n";
 map<int,int> dpos; for(int i=0;i<(int)D10.size();i++) dpos[D10[i]]=i;
 auto restrict_to_D=[&](vector<int> const&p, vector<int>&loc){loc.assign(D10.size(),-1); for(int i=0;i<(int)D10.size();i++){auto it=dpos.find(p[D10[i]]); if(it==dpos.end()) return false; loc[i]=it->second;} return true;};
 auto print_local=[&](string name,vector<int> const&p){vector<int> loc; bool ok=restrict_to_D(p,loc); cout<<"D10_RESTRICT "<<name<<" PRESERVES "<<ok; if(ok){cout<<" PERM";for(int x:loc)cout<<" "<<x; map<int,int>hh; auto ww=cycle_widths(loc,hh); cout<<" CTYPE";for(auto[k,v]:hh)cout<<" "<<k<<":"<<v;} cout<<"\n";};
 print_local("q1",ops[0]); print_local("q2",ops[1]); print_local("q3",ops[2]); print_local("q4sq",ops[3]);
 for(int j=0;j<4;j++) print_local("chrom"+to_string(j),chroms[j]);
 for(int j=0;j<6;j++) print_local("mono"+to_string(j),monos[j]);
 // Canonical ledger phase: each q1-cycle is rooted at its lexicographically least canonical tuple.
 // The dual-forgotten label is the q1-phase j=0,...,4 from that root; p distinguishes the two cycles.
 vector<vector<int>> cyc5;
 set<int> seenD;
 for(int s:D10) if(!seenD.count(s)){
   vector<int> C; int v=s;
   do{ C.push_back(v); seenD.insert(v); v=ops[0][v]; }while(v!=s);
   if(C.size()!=5){ cerr<<"non-5 cycle in D10\n"; return 4; }
   cyc5.push_back(C);
 }
 if(cyc5.size()!=2){ cerr<<"expected two q1 cycles\n"; return 5; }
 auto lex_state_less=[&](int a,int b){ return states[a] < states[b]; };
 vector<vector<int>> rooted;
 for(auto C:cyc5){
   int root=*min_element(C.begin(),C.end(),lex_state_less);
   vector<int> R; int v=root; for(int j=0;j<5;j++){R.push_back(v);v=ops[0][v];}
   if(v!=root){cerr<<"rooted cycle failure\n";return 6;} rooted.push_back(R);
 }
 // Canonically order the two dual sheets by their rooted state keys; this does not affect the pairing.
 if(states[rooted[1][0]] < states[rooted[0][0]]) swap(rooted[0],rooted[1]);
 cout<<"LEDGER_CYCLE_P0"; for(int v:rooted[0]) cout<<" "<<v; cout<<"\n";
 cout<<"LEDGER_CYCLE_P1"; for(int v:rooted[1]) cout<<" "<<v; cout<<"\n";
 vector<int> kapLocal(10,-1); vector<pair<int,int>> kpairs;
 for(int j=0;j<5;j++){
   int a=rooted[0][j], b=rooted[1][j];
   kapLocal[dpos[a]]=dpos[b]; kapLocal[dpos[b]]=dpos[a]; kpairs.push_back({a,b});
 }
 bool kinv=true,kfree=true,kcomm=true;
 for(int i=0;i<10;i++){kinv &= kapLocal[kapLocal[i]]==i; kfree &= kapLocal[i]!=i;}
 vector<int> qloc; if(!restrict_to_D(ops[0],qloc)){cerr<<"q1 restriction vanished\n";return 7;}
 for(int i=0;i<10;i++) kcomm &= kapLocal[qloc[i]]==qloc[kapLocal[i]];
 cout<<"LEDGER_KAPPA_LOCAL";for(int x:kapLocal)cout<<" "<<x;cout<<"\n";
 cout<<"LEDGER_KAPPA_PAIRS";for(auto [a,b]:kpairs)cout<<" ("<<a<<","<<b<<")";cout<<"\n";
 cout<<"LEDGER_KAPPA_CHECK INVOLUTION "<<kinv<<" FREE "<<kfree<<" COMMUTES_Q1 "<<kcomm<<"\n";
 // Match to the five phase candidates in the corresponding verification.
 vector<int>A=rooted[0],B=rooted[1];
 // report phase relative to the earlier D-sorted q1-cycle convention
 vector<int> baseA,baseB; set<int> seen2;
 for(int s:D10) if(!seen2.count(s)){vector<int>C;int v=s;do{C.push_back(v);seen2.insert(v);v=ops[0][v];}while(v!=s); if(baseA.empty())baseA=C;else baseB=C;}
 int phase=-1;
 for(int k=0;k<5;k++){
   bool ok=true; for(int j=0;j<5;j++){
     int a=baseA[j], b=baseB[(j+k)%5];
     if(kapLocal[dpos[a]]!=dpos[b]) ok=false;
   }
   if(ok)phase=k;
 }
 cout<<"LEDGER_PHASE_RELATIVE_OLD "<<phase<<"\n";
 // Projected key multiplicity profile: phase j is the dual-forgotten key.
 cout<<"PROJECTED_KEY_FIBERS";
 for(int j=0;j<5;j++) cout<<" {"<<rooted[0][j]<<","<<rooted[1][j]<<"}";
 cout<<"\nPROJECTED_KEY_MULTIPLICITIES 2 2 2 2 2\n";
 cout<<"D10_PROJECTED_KEY_AUDIT_DONE\n";
 return 0;
}
